...
Go源文件 (.go)
  │
  ▼ internal/compiler/frontend     ← go/parser + go/types 类型检查 + AST 追踪 → IR
  │
  ▼ internal/compiler/ir/optimize  ← CFG/SSA 优化器 (~40 个 pass: SCCP/LICM/CSE/...)
  │
...
- `go generate ./internal/compiler/ir/` — 从 `sonolus.py/sonolus/backend/ops.py` 重新生成 `ops_gen.go`（需要同级目录存在 sonolus.py 克隆）
...
- **IR**: `internal/compiler/ir/` 定义了 CFG 形状的中间表示（`BasicBlock` → `FlowEdge`），直接端口自 sonolus.py 的 `sonolus/backend/`。`IDGen` 是每个编译实例独立的单调 ID 生成器，消除共享可变状态，确保并发编译安全。
- **优化器**: 端口自 sonolus.py 的全部 22 个优化 Pass。支持 3 个优化级别（Minimal/Fast/Standard）。Pass 接口 + ManagedPass 分析声明提供静态依赖验证。
- **模式**: 四种模式（Play/Watch/Preview/Tutorial）通过 `internal/compiler/modecompile` 的泛型框架共享核心装配逻辑，模式特定差异在各自 package 中通过 Callback 常量和 OmitFunc 处理。
...
|-----------|-----------|
| `internal/compiler/ir/optimize/ssa.go` | `backend/optimize/ssa.py` |
| `internal/compiler/ir/optimize/sccp.go` | `backend/optimize/constant_evaluation.py` |
| `internal/compiler/ir/optimize/pipeline.go` | `backend/optimize/optimize.py` |
| `internal/compiler/ir/optimize/inlining.go` | `backend/optimize/inlining.py` |
| `internal/compiler/ir/optimize/cse.go` | `backend/optimize/cse.py` |
| `internal/compiler/ir/optimize/licm.go` | `backend/optimize/licm.py` |
| `internal/compiler/modecompile/modecompile.go` | `build/shared/utils/compile.ts` (JS) |
...
# 仅优化器测试
go test ./internal/compiler/ir/optimize/ -v -count=1

# 更新 golden 文件
go test ./internal/compiler/engine/ -run TestGolden -update

# Fuzz 测试
go test -fuzz=FuzzCompilePlay -fuzztime=30s ./internal/compiler/engine/
```
