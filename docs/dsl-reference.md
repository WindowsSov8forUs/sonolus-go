...
- 静态 pointer alias、`new(T)` 非逃逸局部、nil pointer 零值/转换/比较、有限 runtime pointer target 重绑与 target-set 合并、pointer helper 参数/返回、runtime-selected 数组 pointer 的动态索引、解引用读写、pointer 比较、嵌套动态数组 place 和逐 slot comparable aggregate 比较。callback-local 普通 struct 可保存 pointer alias 字段；零值、字段重绑、pointer receiver、整 struct 赋值、嵌套 struct、值参数与 helper 返回都保持 Go 的别名和按值快照语义。Archetype callback receiver 可作为 pointer 参数传入 package helper，helper 对其字段或 pointee 的写入继续作用于当前实体；只有 helper 重绑定参数自身或取得参数变量地址时，才为该参数建立独立 pointer 快照。包含 pointer/container descriptor 的局部定长数组支持零值/literal、整值复制、常量和动态索引、元素/字段重绑、value range、helper 参数与返回。`new(descriptorStruct)` 与 `&localStruct` 具有独立 aggregate 身份；其 pointer 可保存到局部变量或 struct 字段、动态重绑、比较、传参和返回，解引用写入继续作用于被选中的原对象。nil 解引用在所有 runtime-check 等级终止 callback，`notify` 额外发出诊断。
- Concrete `EntityRef.Get()` view 可保存在 callback-local nullable Archetype pointer：无初始化或显式 `nil` 的局部变量以 entity index `-1` 初始化，可在 runtime 分支中接收同一 Archetype 的 view、重新赋值为 `nil`，并在合流后比较、访问字段或传给 helper。该局部始终保持一个静态 Archetype target，不会退化为普通 pointer、Runtime heap object 或开放类型 entity handle。
...
- 静态 interface concrete type 传播、有限 concrete-type variant、helper 参数/返回、method devirtualization、type switch/type assertion，以及泛型实例中的同类静态分派。Interface 方法返回另一个 interface 时，callback-local result cell 合并各 concrete descriptor 与 bare `nil`；entity-view payload 继续保留 Archetype binding 和 entity index，因此可在调用端继续进行下一层 dispatch。对可能为 `nil` 的 interface 结果 dispatch 与 nil pointer 解引用一致地在所有 runtime-check 等级终止 callback；静态恒为 `nil` 的 interface 结果 dispatch 仍在编译期拒绝。
...
- package-scope `new(T)` 或固定地址复合初始化在 `T` 为编译期有限、固定对象图时分配到当前模式的 semantic memory。同一指针可跨 callback 保持对象身份并通过 pointer receiver/helper 读写。Runtime block 的物理零值直接表示全零对象图；存在非零 scalar 或非 nil pointer/interface 初值时，Compiler 按稳定 storage/offset/name 顺序生成带内部完成 flag 的初始化 CFG，由首个实际执行的 preprocess 恰好提交一次，后续实体 preprocess 不会重置已经修改的对象。具有其他 callback 但未声明 preprocess 的具体 Archetype 会取得 order 0 的合成 preprocess；Tutorial 的 Navigate/Update 也采用同样规则。package object 写权限仍服从实际 block：Play 为 preprocess/updateSequential/touch，Watch 为 preprocess/updateSequential，Preview 仅 preprocess，Tutorial 为 preprocess/navigate/update。Watch `UpdateSpawn` 没有可保证的全局写入初始化阶段，因此只能读取全零初始 package object；引用需要 Runtime 初始化的对象会稳定拒绝。动态堆分配、动态长度数组、container backing 和循环对象图仍拒绝。
