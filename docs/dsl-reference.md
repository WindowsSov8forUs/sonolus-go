...
- package-scope `new(T)` 在 `T` 为编译期有限、固定对象图时分配到当前模式的 semantic memory；同一指针可跨 callback 保持对象身份并通过 pointer receiver/helper 读写。动态堆分配、动态长度数组、container backing 和循环对象图仍拒绝。
