...
func optimizeProject(optimizer *optimize.Optimizer, project *frontend.Project) (*frontend.Project, error) {
	if err := optimizer.Validate(); err != nil {
		return nil, fmt.Errorf("newcompiler: optimize project: %w", err)
	}
	if project == nil {
		return nil, fmt.Errorf("newcompiler: optimize project: project is nil")
	}
	...
	if len(errors) != 0 {
		return nil, fmt.Errorf("newcompiler: optimize callbacks:\n%s", strings.Join(errors, "\n"))
	}
	return result, nil
}
