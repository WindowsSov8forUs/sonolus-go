...
func CloneFunction(function *ir.Function) *ir.Function {
	...
	result := &ir.Function{
		...
		Locals: make([]ir.Type, len(function.Locals)), Blocks: make([]*ir.Block, len(function.Blocks)),
	}
	...
	for i, block := range function.Blocks {
		...
		cloned := &ir.Block{ID: block.ID, Instructions: make([]ir.Instruction, len(block.Instructions))}
