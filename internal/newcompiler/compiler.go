...
import (
	...
	"github.com/WindowsSov8forUs/sonolus-go/internal/newcompiler/backend"
...
func (c *Compiler) Compile(requested ...mode.Mode) (*Artifacts, error) {
	...
	for _, m := range orderedModes {
		if pkg := candidate[m]; pkg != nil {
			...
			if err := parser.Parse(m, pkg); err != nil {
				return nil, fmt.Errorf("newcompiler: parse %s mode: %w", m, err)
			}
			value := stats.Modes[m]
	...
	if err != nil {
		return nil, fmt.Errorf("newcompiler: merge frontend project: %w", err)
	}
...
func normalizeModes(requested []mode.Mode) ([]mode.Mode, error) {
	if len(requested) == 0 {
		return nil, fmt.Errorf("newcompiler: at least one mode is required")
	}
...
func (c *Compiler) loadModes(modes []mode.Mode) (map[mode.Mode]*packages.Package, map[mode.Mode]time.Duration, error) {
	...
	for _, m := range orderedModes {
		...
		if result.err != nil {
			messages = append(messages, fmt.Sprintf("newcompiler: load %s mode: %v", m, result.err))
		} else {
			loaded[m] = result.pkg
		}
