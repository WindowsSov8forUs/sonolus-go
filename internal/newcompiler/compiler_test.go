...
func TestCompilerRejectsUnimplementedOptimizationLevels(t *testing.T) {
	compiler := NewCompiler(Options{Optimization: optimize.LevelStandard}, "./testdata/multimode")
	_, err := compiler.Compile(mode.ModePlay)
	if err == nil || !strings.Contains(err.Error(), "only minimal") {
