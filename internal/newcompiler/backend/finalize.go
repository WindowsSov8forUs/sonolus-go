...
func finalizeFunction(m mode.Mode, function *ir.Function) (snode, error) {
	...
	f := &finalizer{mode: m, function: function, localBases: make([]int, len(function.Locals))}
	next := 0
...
func (f *finalizer) address(place ir.Place) (block int, offset snode, index snode, stride int, shifted bool, err error) {
	...
	case ir.LocalPlace:
		return temporaryMemoryBlock, valueNode(f.localBases[place.ID] + place.Offset), nil, 0, false, nil
	case ir.IndexedLocalPlace:
		idx, e := f.expression(place.Index)
		return temporaryMemoryBlock, valueNode(f.localBases[place.ID] + place.Base + place.Offset), idx, place.Stride, true, e
	case ir.MemoryPlace:
