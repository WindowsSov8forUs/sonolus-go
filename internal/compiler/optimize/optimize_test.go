...
func TestOptimizerRejectsInvalidInputAndLevels(t *testing.T) {
	...
	for _, level := range []Level{LevelMinimal, LevelFast, LevelStandard} {
		if _, err := NewOptimizer(level).Optimize(context, function(returnVoid(0))); err != nil {
			t.Fatalf("level %d error = %v", level, err)
		}
	}
	if _, err := NewOptimizer(Level(99)).Optimize(context, function(returnVoid(0))); err == nil || !strings.Contains(err.Error(), "unknown optimization level") {
		t.Fatalf("unknown level error = %v", err)
	}
}

func TestDefaultOptimizerIsStandard(t *testing.T) {
	optimizer := NewOptimizer(0)
	if optimizer.level != LevelStandard {
		t.Fatalf("default level = %d, want Standard", optimizer.level)
	}
	if len(optimizer.passes) == 0 || optimizer.passes[0].Name() != "CoalesceFlow" {
		t.Fatalf("default pipeline = %#v", optimizer.passes)
	}
}
...
func TestMinimalFoldsControlAndRemovesUnreachable(t *testing.T) {
	...
	result, err := NewOptimizer(LevelMinimal).Optimize(Context{Mode: mode.ModePlay, Callback: "update"}, input)
	if err != nil {
		t.Fatal(err)
	}
