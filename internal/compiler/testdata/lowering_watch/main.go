...
func (*Note) Preprocess() {
	levelMemory := watch.LevelMemory.Get(0)
	watch.LevelMemory.Set(1, levelMemory)
	levelData := watch.LevelData.Get(0)
	watch.LevelData.Set(1, levelData)
	result := watch.Entity.Result()
	result.TargetTime = watch.Time.Now()
	watch.Entity.SetResult(result)
	value := watch.Time.Delta() + watch.Time.Scaled() + watch.Time.Previous() + watch.Time.OffsetAdjusted()
	value += watch.Time.BeatToBPM(1) + watch.Time.BeatToTime(1)
	value += watch.Time.BeatToStartingBeat(1) + watch.Time.BeatToStartingTime(1)
	value += watch.Time.TimeToScaledTime(value) + watch.Time.TimeToStartingScaledTime(value)
	value += watch.Time.TimeToStartingTime(value) + watch.Time.TimeToTimeScale(value)
	_ = watch.Time.Skip()
	...
	skinTransform := watch.SkinTransform.Get()
	watch.SkinTransform.Set(skinTransform)
	particleTransform := watch.ParticleTransform.Get()
	watch.ParticleTransform.Set(particleTransform)
	_, _ = watch.Entity.Info(), watch.Entity.InfoAt(0)
	value += watch.Entity.Key() + watch.CurrentEntityRef[Note]().Index + float64(watch.ArchetypeID[Note]()) + watch.ArchetypeKey[Note]()
